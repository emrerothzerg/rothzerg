...
describe('isEmail test cases', () => {
  it('Should Be Valid', () => {
    validEmails.forEach((email) => expect({ email, valid: isEmail(email) }).toStrictEqual({ email, valid: true }))
  })

  it('Should Be Invalid', () => {
    invalidEmails.forEach((email) => expect({ email, valid: isEmail(email) }).toStrictEqual({ email, valid: false }))
  })
})
